...
plot(f_beha, ylab = "Fishing mortality rate", xlab = "Length (cm)",  main = "Behaviour-dependent fishing mortality ", sub=paste("s=",be_slope1, "|", "Be_n=", be_in1, " | Fbe=1-(1/(1 + e^(s*(Be_score-Be_in)"))


plot(prob1, xlab = "Indivual", ylab = "Surv. Probability (1)", main="p(surv)=1-e^(-(F_max*(F_len+F_be)+m)*t)", sub = paste("fmax=",fmax1, "", "m=", m1))
...
SpBio <- Mbiomass #stock biomass
a <- 10 #recruitmen parameters
b <- 0.5
